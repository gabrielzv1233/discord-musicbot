...
def clear_all():
    ...
    music_queue.clear(); music_history.clear()
...
async def _play_next():
    ...
    now_playing_msg = await text_channel.send(
        embed=embed,
        view=view,
        allowed_mentions=discord.AllowedMentions.none()  # Prevent pinging @user
    )

    now_playing_msg = await text_channel.send(embed=e, view=view)
...
class NowPlayingView(View):
    ...
    async def pauseplay(self, inter, btn):
        if voice_client.is_playing():
            ...
            await _play_next(); btn.label = "⏸ Pause"
        else:
...
async def play(inter: discord.Interaction, query: str):
    ...
    info = await asyncio.to_thread(ydl.extract_info, search_str, False)
    if 'entries' in info:
        info = info['entries'][0]
    item = {
        'url': info['url'], 'webpage_url': info['webpage_url'],
        'title': info['title'], 'duration': info['duration'],
        'uploader': info.get('uploader',''), 'requester': inter.user
    }
...
async def queue_cmd(inter: discord.Interaction):
    ...
    await inter.response.send_message(embed=e, view=QueueView())
...
@app_commands.describe(position="Position (1‑based) in the queue")
